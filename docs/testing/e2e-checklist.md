...
- [ ] 冷启动 App → 跳转 Welcome 页（无闪屏）
- [ ] 点击"生成助记词" → 显示 24 个单词，顺序固定
- [ ] 验证页：随机抽查 3 个词位，填写正确 → 进入 SetPin
- [ ] 验证页：填写错误 → 显示错误提示，不允许继续
- [ ] SetPin：输入 6 位 PIN → 再次确认 → 成功后 `reset` 到 Vault（不可用返回键退回）
- [ ] 返回键在 Vault 页无法退出到 PIN/助记词页
