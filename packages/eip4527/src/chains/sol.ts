...
import type { SolSignRequest, SolSignature } from '../types'

export function decodeSolSignRequest(urString: string): SolSignRequest {
  const { cbor, type } = decodeFromUR(urString)
  if (type !== 'sol-sign-request') {
    throw new Error(`Expected sol-sign-request, got ${type}`)
  }
...
export function encodeSolSignature(sig: SolSignature): string {
  const entries: Array<[Uint8Array, Uint8Array]> = [
    [cborUint(1), cborBytes(sig.requestId)],
    [cborUint(2), cborBytes(sig.signature)],
  ]
  const cbor = cborTag(CBOR_TAGS.SOL_SIGNATURE, cborMap(entries))
  return encodeToUR(cbor, 'sol-signature')
}
