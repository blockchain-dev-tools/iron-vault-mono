---
title: "APDU Framing"
description: "How APDU commands are chunked into BLE frames and reassembled — the OldPhone Wallet frame format."
---
...
OldPhone Wallet implements this framing on both the receive path (host → device, via the Write/WriteCmd characteristics) and the transmit path (device → host, via the Notify characteristic).
